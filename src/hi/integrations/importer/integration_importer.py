...
Each IMPORT-capable integration provides a concrete subclass and
returns an instance from IntegrationGateway.get_importer(). The
framework owns the import workflow (Data Import page, configure
modal, preview, confirm, result modal, post-import placement); the
...
class IntegrationImporter( CapabilityGateway ):
    ...
    def validate_configuration(
        ...
        NOT perform network operations."""
        raise NotImplementedError('Subclasses must override this method')

    def get_candidate_items(self) -> List[CandidateItem]:
        """Synchronously fetch the upstream items that would be
        candidates for import. Implementations filter out items
        already present in HI (by integration_name match)."""
        raise NotImplementedError('Subclasses must override this method')
